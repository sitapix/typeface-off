...
const textEditing = $derived(
  editMode.value && currentFont?.category !== 'mono'
);
...
// Landing on (or navigating between) font-detail pages opens the drawer and
// scrolls the picker to the top. afterNavigate is the idiomatic SvelteKit hook
// for a navigation side-effect — clearer intent than a state-mutating $effect,
// and it fires on the initial load too.
afterNavigate(() => {
...
<AppFrame>
  ...
  {#snippet sidebar()}
    <Sidebar bind:this={sidebarComponent}>
      ...
      <SearchBar />
      {#if !fonts.length}
        <!-- Search excluded every font. The detail list filters by query only,
             so the one lever back is clearing it. -->
        <ListEmptyState
  ...
  {#snippet pageHeader()}
    <!-- Desktop: full controls bar. Mobile: compact bar (swatch row + size stay
         visible, toggles tucked). `category` surfaces the swatch set that
         applies to this font. -->
    <div class="hidden lg:block">
      <Controls category={currentFont?.category} />
    </div>
    <div class="lg:hidden">
      <Controls compact category={currentFont?.category} />
    </div>
  {/snippet}

  {#if currentFont && textEditing}
    <!-- edit mode: the one editable specimen (SSOT) -->
